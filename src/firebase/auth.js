    ...
    if (user) {
      ...
      try {
        ...
        if (profile) {
          ...
          // Normalize: ensure both 'name' and 'displayName' are set
          const normalizedProfile = {
            ...profile,
            name: profile.name || profile.displayName || user.displayName || 'Aspirant',
            displayName: profile.displayName || profile.name || user.displayName || 'Aspirant',
            isAdmin,
          }
          ...
          store.setOnboardingComplete(normalizedProfile.onboardingComplete || isAdmin)
          store.setIsAdmin(isAdmin)
        } else {
          // New user — no profile yet, will complete onboarding
          const displayName = user.displayName || 'Aspirant'
          store.setUser({ uid: user.uid, email: user.email, phone: user.phoneNumber, displayName, photoURL: user.photoURL, isAdmin: emailIsAdmin })
          store.setProfile({ uid: user.uid, email: user.email, name: displayName, displayName, photoURL: user.photoURL || null, onboardingComplete: emailIsAdmin, isAdmin: emailIsAdmin })
        ...
        const displayName = user.displayName || cached?.displayName || cached?.name || 'Aspirant'
        store.setUser({ uid: user.uid, email: user.email, phone: user.phoneNumber, displayName, photoURL: user.photoURL, isAdmin: emailIsAdmin })
        if (cached && cached.uid === user.uid) {
          // Restore exact cached profile — preserves onboardingComplete: true
          store.setProfile({ ...cached, name: cached.name || cached.displayName || displayName, displayName: cached.displayName || cached.name || displayName })
          store.setOnboardingComplete(cached.onboardingComplete || emailIsAdmin)
          store.setIsAdmin(cached.isAdmin || emailIsAdmin)
        } else {
          store.setProfile({ uid: user.uid, email: user.email, name: displayName, displayName, onboardingComplete: emailIsAdmin, isAdmin: emailIsAdmin })
          store.setOnboardingComplete(emailIsAdmin)
          store.setIsAdmin(emailIsAdmin)
        }
      } finally {
