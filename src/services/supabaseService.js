  ...
  try {
    ...
    if (response.ok) {
      ...
      return data.map(t => ({
        ...
        pattern: t.pattern || 'MCQ',
        negativeMarking: t.negative_marking || -0.25,
        marksPerQuestion: t.marks_per_question || 1,
        syllabus: ['General'],
        attempts: 0,
        avgScore: 0,
...
/**
 * Fetches payment history for a user from Supabase payments table.
 */
export async function getPaymentHistory(userId) {
  const { url: supabaseUrl, anonKey: supabaseAnonKey } = getSupabaseCredentials()
  const cleanUrl = supabaseUrl.trim().replace(/\/$/, '')
  const dbUrl = `${cleanUrl}/rest/v1/payments?user_id=eq.${userId}&select=*&order=created_at.desc`
  ...
  try {
    ...
    console.error('[Supabase DB] getPaymentHistory failed:', err)
  }
  return []
}
