...
export default function Courses() {
  ...
  return (
    <div className="page animate-fade-in">
      ...
      {(() => {
        const renderCourseCard = (course, isNetflixRow = false) => {
          ...
          return (
            <div 
              key={course.id} 
              className={`prepinsta-card ${isNetflixRow ? 'netflix-card' : ''} ${isTarget ? 'prepinsta-card-purple' : ''}`} 
              style={{ 
                overflow: 'hidden', 
                display: 'flex', 
            ...
            >
              {/* Course banner */}
              <div style={{ background: `linear-gradient(135deg, ${accentColor}18, ${accentColor}06)`, borderBottom: `1px solid rgba(255,255,255,0.03)`, padding: '20px 18px', position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: -10, right: -10, fontSize: '4rem', opacity: 0.08 }}>{course.icon}</div>
                <div style={{ fontSize: '1.6rem', marginBottom: 8 }}>{course.icon}</div>
                <div style={{ fontWeight: 900, fontSize: '1rem', marginBottom: 4, color: 'white', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{course.title}</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', fontWeight: 650 }}>{course.exam}</div>
              ...
              <div style={{ padding: 16, flex: 1, display: 'flex', flexDirection: 'column', gap: 12, justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-2)', display: 'inline-flex', alignItems: 'center', gap: 4, fontWeight: 600 }}>
                    <Play size={11} color={accentColor} /> {course.lessons} lessons
                  </span>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-2)', display: 'inline-flex', alignItems: 'center', gap: 4, fontWeight: 600 }}>
                    <Clock size={11} color={accentColor} /> {course.hours}h
                  </span>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-2)', display: 'inline-flex', alignItems: 'center', gap: 3, fontWeight: 600 }}>
                    <Star size={11} fill="var(--amber)" color="var(--amber)" /> {course.rating}
                  </span>
                </div>

                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {course.topics.slice(0, 3).map(t => (
                    <span key={t} style={{ fontSize: '0.65rem', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: 30, padding: '2px 8px', color: 'var(--text-2)', fontWeight: 600 }}>{t}</span>
                  ))}
                  {course.topics.length > 3 && (
                    <span style={{ fontSize: '0.65rem', color: 'var(--text-4)', fontWeight: 600, alignSelf: 'center' }}>+{course.topics.length - 3}</span>
                  )}
                </div>

                {progress !== null && (
                  <div style={{ margin: '2px 0', padding: '10px', background: 'rgba(255,255,255,0.01)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.03)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: '0.72rem' }}>
                      <span style={{ color: 'var(--text-3)', fontWeight: 700 }}>Coverage</span>
                      <span style={{ fontWeight: 900, color: accentColor }}>{progress}%</span>
                    </div>
                    <div className="progress-bar" style={{ height: 4, background: 'rgba(255,255,255,0.06)', borderRadius: 2 }}>
                      <div className="progress-fill" style={{ width: `${progress}%`, background: accentColor, height: 4 }} />
                    </div>
                  </div>
                )}
                ...
                >
                  {course.free ? <><Play size={12} /> Start Free Prep</> : <><BookOpen size={12} /> Resume Course</>}
                </button>
              </div>
            </div>
          )
        ...
        if (filter === 'all' && !showFreeOnly) {
          const targetCourses = COURSES.filter(c => isUserPrimaryTarget(c))
          const generalCourses = COURSES.filter(c => c.id === 'gk_crash' || c.id === 'current_affairs_monthly' || c.id === 'reasoning_tricks')
          const coreCourses = COURSES.filter(c => c.id !== 'gk_crash' && c.id !== 'current_affairs_monthly' && c.id !== 'reasoning_tricks' && !isUserPrimaryTarget(c))
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
              {targetCourses.length > 0 && (
                <div className="netflix-row-container">
                  <div className="netflix-row-header">
                    <div className="netflix-row-title">🎯 Recommended for Your Target</div>
                  </div>
                  <div className="netflix-row">
                    {targetCourses.map(course => renderCourseCard(course, true))}
                  </div>
                </div>
              )}
              ...
              <div className="netflix-row-container">
                <div className="netflix-row-header">
                  <div className="netflix-row-title">🧠 General Aptitude & GK Foundations</div>
                </div>
                <div className="netflix-row">
                  {generalCourses.map(course => renderCourseCard(course, true))}
                </div>
              </div>

              <div className="netflix-row-container">
                <div className="netflix-row-header">
                  <div className="netflix-row-title">🎓 Core Video Syllabus Tracks</div>
                </div>
                <div className="netflix-row">
        ...
        return (
          <div className="grid-3" style={{ gap: 20 }}>
            {filtered.map(course => renderCourseCard(course, false))}
          </div>
        )
      })()}
    </div>
