...
const OTP_COOLDOWN = 30 // seconds before resend is allowed
...
export default function Auth() {
  ...
  useEffect(() => {
    ...
    if (user) {
      const { isAdmin, onboardingComplete: oc } = useUserStore.getState()
      if (isAdmin) navigate('/admin', { replace: true })
      else if (oc) navigate('/app/home', { replace: true })
  ...
  useEffect(() => {
    if (method !== 'phone' || recaptchaReady.current) return
    // Small delay to ensure DOM is ready
    const t = setTimeout(() => {
      const ok = setupRecaptcha('recaptcha-container')
      if (ok) recaptchaReady.current = true
    }, 100)
  ...
  const handlePhoneStep1 = async () => {
    ...
    try {
      ...
      console.warn('[Auth] Phone OTP error (demo fallback):', err.message)
      setIsDemoMode(true)
      setStep(2)
      setOtp('')
      startCooldown()
      toast.success('Demo mode active — use OTP 123456')
    }
    setLoading(false)
  }
  ...
  const handlePhoneOTP = async () => {
    ...
    try {
      ...
      const code = err?.code
      if (code === 'auth/invalid-verification-code') toast.error('Wrong OTP. Check and try again.')
      else if (code === 'auth/code-expired') toast.error('OTP expired. Please request a new one.')
  ...
  const handleEmailAuth = async () => {
    ...
    if (mode === 'signup' && !name.trim()) { toast.error('Enter your full name'); return }
    setLoading(true)
    try {
      if (mode === 'signup') {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // Navigation handled by onAuthStateChanged observer in main.jsx
    } catch (err) {
      if (err?.code === 'auth/popup-closed-by-user') toast.error('Sign-in cancelled.')
      else if (err?.code === 'auth/network-request-failed') toast.error('Network error. Check your connection.')
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: 'var(--bg)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 40, position: 'relative', overflow: 'hidden' }}>
        {/* Invisible recaptcha anchor — always in DOM when phone tab active */}
        <div id="recaptcha-container" style={{ position: 'absolute', bottom: 0 }} />

        <div style={{ position: 'absolute', top: '-20%', right: '-20%', width: 500, height: 500, background: 'var(--purple)', borderRadius: '50%', filter: 'blur(120px)', opacity: 0.18, pointerEvents: 'none' }} />
        ...
        <div style={{ width: '100%', maxWidth: 420, position: 'relative', zIndex: 1 }}>
          ...
          <div className="tabs" style={{ marginBottom: 20 }}>
            <button className={`tab ${method === 'phone' ? 'active' : ''}`} onClick={() => { setMethod('phone'); setStep(1); setOtp('') }}>
            ...
            {method === 'phone' && step === 2 && (
              <div className="form-group">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                  <button type="button" onClick={() => { setStep(1); setOtp(''); setIsDemoMode(false) }}
                    style={{ background: 'none', border: 'none', color: 'var(--cyan)', fontSize: '0.82rem', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>
                    ← Change number
                  </button>
                  <button type="button" onClick={handleResendOTP} disabled={resendCooldown > 0}
                    style={{ background: 'none', border: 'none', color: resendCooldown > 0 ? 'var(--text-4)' : 'var(--cyan)', fontSize: '0.82rem', cursor: resendCooldown > 0 ? 'default' : 'pointer', fontFamily: 'inherit', padding: 0 }}>
                    {resendCooldown > 0 ? `Resend in ${resendCooldown}s` : 'Resend OTP'}
                  </button>
                </div>
                {isDemoMode && (
                  <div style={{ fontSize: '0.78rem', color: 'var(--amber)', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 'var(--r-sm)', padding: '8px 12px', marginTop: 8 }}>
                    💡 Demo mode active — Firebase phone auth not configured. Use OTP <strong>123456</strong>
                  </div>
                )}
              </div>
            )}
                ...
                <div className="form-group">
                  <label className="form-label">
                    ...
                    {mode === 'login' && (
                      ...
                      }} style={{ background: 'none', border: 'none', color: 'var(--cyan)', fontSize: '0.78rem', cursor: 'pointer', fontFamily: 'inherit', marginLeft: 'auto', float: 'right' }}>
                        Forgot password?
                      </button>
                    )}
                  </label>
          ...
          <p style={{ textAlign: 'center', marginTop: 20, fontSize: '0.88rem', color: 'var(--text-3)' }}>
            ...
            <button onClick={() => setMode(mode === 'login' ? 'signup' : 'login')}
              style={{ background: 'none', border: 'none', color: 'var(--cyan)', cursor: 'pointer', fontWeight: 600, fontFamily: 'inherit' }}>
      ...
      <div className="auth-right" style={{ flex: 1, background: 'var(--bg-2)', borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 48, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', bottom: '-10%', right: '-10%', width: 400, height: 400, background: 'var(--cyan)', borderRadius: '50%', filter: 'blur(100px)', opacity: 0.12, pointerEvents: 'none' }} />
